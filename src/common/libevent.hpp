...
template <
    decltype(evutil_parse_sockaddr_port) func = ::evutil_parse_sockaddr_port>
...
class Bufferevent : public NonCopyable, public NonMovable {
  public:
    ...
    template <
        decltype(bufferevent_socket_new) construct = ::bufferevent_socket_new,
    ...
    template <
        decltype(bufferevent_write_buffer) func = ::bufferevent_write_buffer>
    ...
    template <
        decltype(bufferevent_read_buffer) func = ::bufferevent_read_buffer>
    ...
    template <
        decltype(bufferevent_set_timeouts) func = ::bufferevent_set_timeouts>
...
void evdns_base_resolve_ipv4(Var<evdns_base> base, std::string name,
    ...
    if (resolve(base.get(), name.c_str(), flags, handle_resolve, cb) ==
        nullptr) {
